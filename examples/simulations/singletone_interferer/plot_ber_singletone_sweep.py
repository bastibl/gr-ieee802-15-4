...
if __name__ == "__main__":
    ...
    f_oqpsk = np.arange(-fs_oqpsk/2, fs_oqpsk/2, 5e5)
    f_css_slow = np.arange(-fs_css/2, fs_css/2, 5e5)
    f_css_fast = np.arange(-13e6, 13e6, 5e5)

    plt.rcParams.update({'font.size': 14})
    plt.rcParams.update({'axes.labelsize': 'large'})
    plt.rcParams.update({'axes.labelsize': 'large'})

    plt.semilogy(f_css_slow/1e6, css_slow, label='CSS 250 kb/s', marker='o')
    plt.semilogy(f_css_fast/1e6, css_fast, label='CSS 1 Mb/s', marker='v')
